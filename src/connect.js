...
function canMakeQr(options) {
  return options.hasPermission(PERMISSIONS.MANAGE_INVITES);
}
...
function bindDirectoryActions(root, options) {
  root.querySelectorAll("[data-switch-church]").forEach((button) => button.addEventListener("click", async () => {
    try {
      ...
      options.toast("Could not switch churches", error?.message || "Try again.", "error");
    }
  }));
...
async function openConversation(connection, options) {
  ...
  modal.addEventListener("click", (event) => {
    if (event.target === modal || event.target.closest("[data-close-connect]")) close();
  });
...
async function renderNetwork(options) {
  ...
  try {
    ...
    const qrPanel = canMakeQr(options)
    ...
    root.innerHTML = `
      ${manageChurch ? `<div class="connect-settings-grid">
        ...
        ${qrPanel}
      </div>` : qrPanel ? `<div class="connect-settings-grid">${qrPanel}</div>` : ""}
    ...
    bindNetwork(options, connections, networkDirectory);
  } catch (error) {
    root.innerHTML = emptyState("Could not load Church Network", error?.message || "Try again in a moment.");
  }
}

function bindNetwork(options, connections, initialDirectory) {
  document.querySelector("#save-discovery-settings")?.addEventListener("click", async (event) => {
    ...
    try {
      ...
      options.toast("Could not save discovery settings", error?.message || "Try again.", "error");
      button.disabled = false;
    }
  });
  ...
  document.querySelector("#save-network-settings")?.addEventListener("click", async (event) => {
    ...
    try {
      ...
      options.toast("Could not update Church Network", error?.message || "Try again.", "error");
      button.disabled = false;
    }
  });
    ...
    try {
      ...
      output?.querySelector("[data-copy-qr]")?.addEventListener("click", async () => {
        ...
        try {
          ...
          options.toast("Join link copied", "You can paste it into a bulletin, text, email, or social post.", "success");
        } catch (_) {
          options.toast("Copy the link manually", "Your browser did not allow clipboard access.");
        }
      });
    } catch (error) {
      options.toast("Could not create QR code", error?.message || "Try again.", "error");
    } finally {
      button.disabled = false;
      button.textContent = "Generate another QR join code";
    }
  });
  ...
  document.querySelectorAll("[data-review-join]").forEach((button) => button.addEventListener("click", async () => {
    try {
      ...
      options.toast("Could not review request", error?.message || "Try again.", "error");
    }
  }));

  const bindNetworkSearchResults = (root, renderedResults) => {
    root?.querySelectorAll("[data-network-connect]").forEach((button) => button.addEventListener("click", async () => {
      const church = renderedResults.find((item) => item.churchId === button.dataset.networkConnect);
      ...
      try {
        ...
        options.toast("Could not connect churches", error?.message || "Try again.", "error");
      }
    }));
  };
  bindNetworkSearchResults(document.querySelector("#network-search-results"), initialDirectory);
  ...
  document.querySelector("#network-search-submit")?.addEventListener("click", async () => {
    ...
    try {
      ...
      bindNetworkSearchResults(root, results);
    } catch (error) {
      root.innerHTML = emptyState("Search failed", error?.message || "Try again.");
    }
  });
  ...
  document.querySelectorAll("[data-respond-connection]").forEach((button) => button.addEventListener("click", async () => {
    try {
      ...
      options.toast("Could not respond", error?.message || "Try again.", "error");
    }
  }));
  ...
  document.querySelectorAll("[data-remove-connection]").forEach((button) => button.addEventListener("click", async () => {
    ...
    try {
      await removeChurchConnection(connection.id);
      options.toast("Churches disconnected", "The network conversation was removed.", "success");
      await renderNetwork(options);
    } catch (error) {
      options.toast("Could not disconnect", error?.message || "Try again.", "error");
    }
  }));
}
...
export function decorateOnboardingDiscovery(options) {
  ...
  const copy = document.querySelector(".onboarding-copy p");
  if (copy) copy.textContent = "Church Chatter is organized around real congregations. Create a church community, enter an invitation code, or search for a congregation that chose to be discoverable.";
...
export async function showQrJoinConfirmation(code, options) {
  ...
  return new Promise((resolve) => {
    ...
    modal.querySelector("[data-qr-confirm]")?.addEventListener("click", async (event) => {
      ...
      try {
        await joinChurch(options.state.user, code);
        finish(true);
      } catch (error) {
        options.toast("Could not join congregation", error?.message || "Try again.", "error");
        event.currentTarget.disabled = false;
        event.currentTarget.textContent = "Join congregation";
      }
    });
  });
}
